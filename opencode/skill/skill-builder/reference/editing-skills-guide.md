...
### 4. Add CLI and Bun Patterns

**Problem:** Skill doesn't emphasize modern tooling

**Solution:** Add specific CLI commands and Bun examples

**What to add:**
- CLI tools section with specific commands
- Bun script examples (not Python)
...
See the Bun and CLI patterns reference in `./reference/` for more examples.
```
...
└── script-template.ts      # Bun script skeleton
```

**Scripts:**
```
scripts/
├── process-data.ts         # Data processing utility
├── validate-output.ts      # Validation script
└── transform-files.ts      # File transformation
```
...
See `./scripts/process-csv.ts` for complete implementation.
```
...
| Python scripts | Replace with Bun | scripts/*.ts |

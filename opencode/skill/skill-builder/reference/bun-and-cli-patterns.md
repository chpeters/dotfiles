# Bun and CLI Patterns Reference

## Philosophy: CLI-First, Bun for Complex Logic

**Emphasize modern tooling:**
- Use CLI tools liberally (gh, aws, bun, git, jq, etc.)
- Encourage global Bun package installation when useful
- Script with Bun v1+ (not Python)
...
```bash
# Sequential with &&
gh pr view 123 --json commits && git diff main...pr-123 && bun test
...
// Fetch API (built-in Bun runtime)
...
**Benefits of Bun for Claude Code skills:**

1. **Consistency**: Same runtime as the Bun/TypeScript ecosystem
2. **Modern async/await**: Clean, readable async code
3. **Built-in JSON**: Native JSON parsing and stringification
4. **Active maintenance**: Bun v1+ is actively supported
...
- Less consistent with the Bun/TypeScript ecosystem

## Global Bun Packages to Consider

**Data processing:**
```bash
bun add -g csv-parse csv-stringify
bun add -g lodash
bun add -g ramda
```
...
if (!prNumber) {
  console.error('Usage: analyze-pr.ts <pr-number>');
  process.exit(1);
}
...
if (!functionName) {
  console.error('Usage: analyze-logs.ts <function-name> [minutes]');
  process.exit(1);
}
...
3. **Make scripts executable** - Add `#!/usr/bin/env bun` shebang
